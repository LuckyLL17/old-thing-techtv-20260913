...
| 7 | 个人中心 | 我发布的 / 收藏 / 尝试记录 / 作品 / 改造总计 / 站内通知 / 成就徽章墙 |
| 8 | 统计看板 + 管理 | 平台总数、TOP10 热门教程、最活跃用户、分类占比、月度趋势、审计日志、管理员操作审计统计、徽章发放流水追踪 |
| 9 | 成就徽章 | 首发教程、作品点赞（10/50/100）、连续发布（3/6 月）、收藏他人教程（10/50）共 8 枚；事件触发自动颁发 + 定时对账补发，已得不收回、同条件不重发，个人中心展示已获得/未获得状态与实时进度 |
...
| GET  | `/badges` | 我的徽章墙：全部 8 枚定义 + 已获得/未获得状态 + 当前进度 |

### 成就徽章

| 方法 | 路径 | 说明 | 鉴权 |
|---|---|---|---|
| GET  | `/me/badges` | 徽章墙：`badges[]`（code/name/icon/description/condition/threshold/earned/progress/awarded_at）+ `earned_count/total_count` | 是 |
| GET  | `/users/:id/badges` | 他人主页只返回其已获得的徽章 | 可选 |
| GET  | `/admin/badge-logs` | 徽章判定/发放流水，支持 `user_id/badge_code/result/event` 过滤分页 | 是 |

徽章 code 共 8 枚：`first_publish_1`（首次发布教程）、`project_likes_10/50/100`（单个作品点赞门槛）、`publish_streak_3/6`（连续 N 个月发布）、`favorite_others_10/50`（收藏他人教程门槛）。

颁发规则：

- **自动颁发**：教程发布（含草稿转发布）、作品被点赞、收藏教程后实时判定；满足条件立即发放。
- **定时对账**：统计 worker 每 10 分钟全量扫描一次（启动时也会执行），补发历史用户或任何漏发的徽章，事件标记为 `sweep`。
- **不收回**：已获得的徽章不因点赞数下降、取消收藏、删除教程等数据回落而失效。
- **不重发**：`(user_id, badge_code)` 唯一索引兜底，同一条件重复达成只产生 `duplicate` 流水，不会发第二枚。
- **可追踪**：每次判定都写 `badge_award_logs`，`result` 为 `granted/duplicate/not_met`，并携带判定瞬间的指标快照（JSON）；颁发同时推送系统通知。
- 发布教程与收藏接口的响应里额外带 `data.badges`（本次新获得的徽章数组），供前端即时提示。
...
| GET | `/badge-logs` | 徽章判定/发放流水追踪，支持结果/事件/用户/徽章过滤 |
